import React from 'react';

// Removed imports for 'next/link' (and 'next/image', though not present) to fix "Could not resolve" errors.

const navLinks = [
    { title: "Home", icon: "fas fa-home", href: "/" },
    { title: "About Me", icon: "fas fa-user", href: "/aboutme" },
    { title: "Resume/CV", icon: "fas fa-file-alt", href: "/eduexp" },
    { title: "Portfolio/Works", icon: "fas fa-briefcase", href: "/projects" },
    { title: "Specializations", icon: "fas fa-comments", href: "/spec" },
    { title: "Services/Skills", icon: "fas fa-cogs", href: "/skills" },
    { title: "Contact", icon: "fas fa-envelope", href: "/contactme" },
];

const PRIMARY_COLOR = '#FF5C00'; 
const BACKGROUND_COLOR = '#1D1C1D';
const CARD_BACKGROUND = '#1a1a1a';

// Defining the styles and config as string variables (Corrected from the user's provided block)
const customStyles = `
    body {
        font-family: 'Inter', sans-serif;
        background-color: ${BACKGROUND_COLOR};
        color: #f0f0f0;
    }
    .text-primary-orange {
        color: ${PRIMARY_COLOR};
    }
    .bg-primary-orange {
        background-color: ${PRIMARY_COLOR};
    }
    .shadow-primary-orange {
        box-shadow: 0 0 15px 5px ${PRIMARY_COLOR}33;
    }
    .sidebar-active {
        color: ${PRIMARY_COLOR};
        background-color: #2a2a2a;
    }
    /* Custom focus style for inputs */
    .input-field:focus {
        outline: none;
        border-color: ${PRIMARY_COLOR};
    }
`;

const tailwindConfig = `
    tailwind.config = {
        theme: {
            extend: {
                colors: {
                    'dark-card': '${CARD_BACKGROUND}',
                    'primary-orange': '${PRIMARY_COLOR}',
                }
            }
        }
    }
`;


// Renamed the function to reflect that this is a contact page
export default function ContactPage() { 
    return (
        <>
            <title>Jirawad - Contact Me</title>
            <meta name="viewport" content="width=device-width, initial-scale=1.0" />

            {/* Note: In a real Next.js app, external scripts/styles should be managed in _document.js or layout.tsx */}
            <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css" />
            <script src="https://cdn.tailwindcss.com" async></script>
            
            {/* Inject custom CSS and Tailwind config using dangerouslySetInnerHTML */}
            <style dangerouslySetInnerHTML={{ __html: customStyles }} />
            <script dangerouslySetInnerHTML={{ __html: tailwindConfig }} />
            
            <div className="p-4 md:p-8 lg:p-12 min-h-screen flex items-center justify-center">

                <div className="main-container w-full max-w-7xl">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">

                        {/* Profile Card / Sidebar */}
                        <div className="profile-card lg:col-span-1 p-6 md:p-8 rounded-2xl shadow-2xl bg-dark-card border border-gray-700/50">
                            <h2 className="text-3xl font-bold mb-4"><span className="text-primary-orange">J</span>irawad</h2>
                            
                            <div className="mb-6 overflow-hidden rounded-2xl border-4 border-gray-700 w-48 h-48 mx-auto relative">
                                {/* Using standard <img> tag */}
                                <img 
                                    src="/avatar.png" 
                                    alt="Profile Avatar" 
                                    className="w-full h-full object-cover"
                                />
                            </div>

                            <div className="text-center space-y-2 mb-8">
                                <p className="text-lg font-medium">STU ID: 6652410033</p>
                                <p className="text-sm text-gray-400">Fullstack Web Developer</p>
                            </div>

                            <div className="flex justify-center space-x-4 mb-10 text-gray-400">
                                <a href="#" className="hover:text-primary-orange transition duration-300 transform hover:scale-110"><i className="fab fa-facebook-f text-xl"></i></a>
                                <a href="#" className="hover:text-primary-orange transition duration-300 transform hover:scale-110"><i className="fab fa-twitter text-xl"></i></a>
                                <a href="#" className="hover:text-primary-orange transition duration-300 transform hover:scale-110"><i className="fab fa-instagram text-xl"></i></a>
                                <a href="#" className="hover:text-primary-orange transition duration-300 transform hover:scale-110"><i className="fab fa-linkedin-in text-xl"></i></a>
                                <a href="#" className="hover:text-primary-orange transition duration-300 transform hover:scale-110"><i className="fab fa-google text-xl"></i></a>
                            </div>

                            <button className="w-full py-3 rounded-xl bg-primary-orange text-black font-bold text-lg shadow-lg hover:shadow-primary-orange/50 transition duration-300 transform hover:scale-[1.02]">
                                HIRE ME !
                            </button>
                        </div>

                        {/* Main Content Area (Contact Form) */}
                        <div className="main-content lg:col-span-2 p-6 md:p-8 rounded-2xl">
                            
                            <h1 className="text-4xl md:text-5xl font-extrabold leading-tight mb-2">
                                Contact <span className="text-primary-orange">Me</span>
                            </h1>
                            <p className="text-gray-400 text-lg mb-8">Let&apos;s get in touch!</p>

                            <form className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="relative">
                                        <input 
                                            type="email" 
                                            id="email" 
                                            className="input-field w-full pt-6 pb-2 bg-transparent border-b border-gray-600 focus:border-primary-orange transition duration-200 text-white" 
                                            placeholder=" "
                                        />
                                        <label 
                                            htmlFor="email" 
                                            className="absolute left-0 top-0 text-sm text-gray-400 transition-all transform -translate-y-1/2 pt-6 pointer-events-none"
                                        >
                                            Email
                                        </label>
                                    </div>
                                    <div className="relative">
                                        <input 
                                            type="tel" 
                                            id="phone" 
                                            className="input-field w-full pt-6 pb-2 bg-transparent border-b border-gray-600 focus:border-primary-orange transition duration-200 text-white" 
                                            placeholder=" "
                                        />
                                        <label 
                                            htmlFor="phone" 
                                            className="absolute left-0 top-0 text-sm text-gray-400 transition-all transform -translate-y-1/2 pt-6 pointer-events-none"
                                        >
                                            Phone
                                        </label>
                                    </div>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="relative">
                                        <input 
                                            type="text" 
                                            id="name" 
                                            className="input-field w-full pt-6 pb-2 bg-transparent border-b border-gray-600 focus:border-primary-orange transition duration-200 text-white" 
                                            placeholder=" "
                                        />
                                        <label 
                                            htmlFor="name" 
                                            className="absolute left-0 top-0 text-sm text-gray-400 transition-all transform -translate-y-1/2 pt-6 pointer-events-none"
                                        >
                                            Name
                                        </label>
                                    </div>
                                    <div className="relative">
                                        <input 
                                            type="text" 
                                            id="address" 
                                            className="input-field w-full pt-6 pb-2 bg-transparent border-b border-gray-600 focus:border-primary-orange transition duration-200 text-white" 
                                            placeholder=" "
                                        />
                                        <label 
                                            htmlFor="address" 
                                            className="absolute left-0 top-0 text-sm text-gray-400 transition-all transform -translate-y-1/2 pt-6 pointer-events-none"
                                        >
                                            Address
                                        </label>
                                    </div>
                                </div>

                                <div className="relative pt-4">
                                    <textarea 
                                        id="content" 
                                        className="input-field w-full pt-6 pb-2 bg-transparent border-b border-gray-600 focus:border-primary-orange transition duration-200 text-white resize-none" 
                                        placeholder=" "
                                    ></textarea>
                                    <label 
                                        htmlFor="content" 
                                        className="absolute left-0 top-0 text-sm text-gray-400 transition-all transform -translate-y-1/2 pt-6 pointer-events-none"
                                    >
                                        Content
                                    </label>
                                </div>

                                <div className="flex flex-col space-y-4">
                                    <div className="flex items-center">
                                        <input type="checkbox" id="newsletter" className="form-checkbox text-primary-orange bg-gray-700 border-gray-600 rounded" />
                                        <label htmlFor="newsletter" className="ml-2 text-gray-400 text-sm">I would like to receive the newsletter.</label>
                                    </div>
                                    
                                    <button 
                                        type="submit" 
                                        className="py-3 rounded-xl bg-primary-orange text-black font-bold text-lg shadow-lg hover:shadow-primary-orange/50 transition duration-300 transform hover:scale-[1.01] w-full md:w-auto px-10 self-start"
                                    >
                                        Submit
                                    </button>
                                </div>
                            </form>

                        </div>
                        
                    </div>
                </div>
                
                {/* Fixed Navigation Sidebar */}
                <div className="fixed right-0 top-1/2 transform -translate-y-1/2 p-3 bg-dark-card/80 backdrop-blur-sm rounded-full border border-gray-700/50 hidden lg:block shadow-2xl">
                    <nav className="flex flex-col space-y-4 text-gray-400">
                        {navLinks.map((link, index) => (
                            // Replaced <Link> with <a>
                            <a 
                              key={index}
                              href={link.href} 
                              className={`p-2 rounded-full hover:text-primary-orange transition duration-200 ${index === 6 ? 'sidebar-active' : 'hover:bg-gray-700'}`} // Index 6 is 'Contact'
                              title={link.title}
                            >
                              <i className={`${link.icon} text-xl`}></i>
                            </a>
                        ))}
                    </nav>
                </div>
            </div>
        </>
    );
}
